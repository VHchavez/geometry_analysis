...
def test_molecule_set_coordinates(water_molecule):
    """Test that the bond list is rebuilt when we resent coordinates."""

    num_bonds = len(water_molecule.bonds)
    
    assert num_bonds == 2

    new_coordinates =  np.array([[10,0,0], [0,0,0], [-2,0,0]])
...
def test_angle_90():
    ...
    calculated_angle_degree = geometry_analysis.calculate_angle(r1,r2,r3, True)
